...
def loadvideo(fname, fps=None, stride=15, max_duration=None):
    vr = VideoReader(fname, num_threads=1, ctx=cpu(0))
    duration = len(vr) / vr.get_avg_fps()
    if fps is not None:
        num_sampled_frames = round(duration * fps)
        all_index = np.linspace(
            0, len(vr)-1, num=num_sampled_frames).round().astype(np.int32)
        print(len())
        ...
        if max_duration is not None:
            all_index = all_index[:round(
                max_duration * all_index.shape[0] / duration)]
    vr.seek(0)
...
if __name__ == '__main__':
    video = loadvideo(
        fname='/root/autodl-tmp/data/SumMe/videos/Cooking.mp4',
        fps=None,
        stride=15,
        max_duration=None
    )
    ...
    #         video_path, None, stride=args.stride, batch_size=args.batch_size)
