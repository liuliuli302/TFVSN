...
def main(dataset_dir, result_dir):
    ...
    for dataset in json_files:
        result_file_name = str(dataset).split(
            '/')[-1].split('.')[0] + "_result.json"
        ...
        for sample in tqdm(dataset, desc="Samples", leave=False):
            ...
            images = image_processor.preprocess(images, return_tensors="pt")[
            ...
            input_ids = tokenizer_image_token(
                prompt2, tokenizer, IMAGE_TOKEN_INDEX, return_tensors="pt").unsqueeze(0).to(device)
            cont = model.generate(
                input_ids,
                images=images,
                modalities=["video"],
            ...
            text_outputs2 = tokenizer.batch_decode(
                cont, skip_special_tokens=True)[0].strip()
...
if __name__ == "__main__":
    args = parse_args()
    main(args.dataset_save_dir, args.result_dir)
